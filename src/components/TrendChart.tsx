...
import type { QuotaSnapshot, ModelUsage } from "../types";
...
export default function TrendChart({ accountId, days = 7 }: TrendChartProps) {
  ...
  const renderAccountView = () => {
    ...
    const chartData = snapshots.map((snapshot: QuotaSnapshot) => ({
  ...
  const renderModelView = () => {
    ...
    modelUsage.forEach((usage: ModelUsage) => {
  ...
  return (
    <div className="space-y-3">
      ...
      {view === "account" ? renderAccountView() : renderModelView()}
    </div>
  );
}
